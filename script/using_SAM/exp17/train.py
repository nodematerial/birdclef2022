...
class WaveformDataset(torchdata.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        psuedo_labels = np.zeros(len(CFG['target_columns']), dtype=float)

        if ebird_code in CFG['target_columns']:
            labels[CFG['target_columns'].index(ebird_code)] = 1
        if CFG['second_label']:
            for code in second_codes:
                if code in CFG['target_columns']:
                    labels[CFG['target_columns'].index(code)] = 1
        if CFG['psuedo_label']:
            for code in psuedo_code:
                if code in CFG['target_columns']:
                    psuedo_labels[CFG['target_columns'].index(code)] = 1
        labels = np.where(np.logical_or(labels, psuedo_labels), 1, 0)
        return  y, labels
...
def training(logger, fold):
    ...
    psuedo = pd.read_csv(CFG['psuedo_csv'])['birds']
    train = pd.concat([train, psuedo], axis = 1)
    train.secondary_labels = train.secondary_labels.apply(eval)
    train.birds = train.birds.apply(eval)
